...
- If you are writing a UWP application, and are implementing the ability to invite users to a game, there are instructions on the `.appxmanifest` changes necessary in [Multiplayer invites for UWP games](../../../features/multiplayer/invites/concepts/live-multiplayer-invite-uwp.md). This was previously discussed on the [forums](https://forums.xboxlive.com) and in [Porting Xbox Live code from XDK to UWP](../../../test-release/live-port-xbl-code-from-xdk-to-uwp.md).
