...
* The stock .pfx file provided by Unity won't have the correct identity, so either delete it from the disk and remove the line in the `.csproj` that references it, or right-click the Project in Visual Studio and then click **Store** \| **Associate App with the Store**, which will place down a proper .pfx file.
  Then go back to Unity, click on **Build Settings** on the **Universal Windows Platform** player, and click the **PFX** button to replace the .pfx file with the .pfx you got from Visual Studio's **Associate App with the Store** action.
