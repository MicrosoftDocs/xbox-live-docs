...
# Multiplayer sessions in Multiplayer Session Directory (MPSD)

## Session Overview

A *session* in Multiplayer Session Directory (MPSD) has a session name and is identified as an instance of a session template. A *session template* is a JSON document that provides default settings for the session. The template is part of a service configuration with a service configuration identifier (SCID), which is a GUID. This template can be found on [Xbox Developer Portal (XDP)](https://xdp.xboxlive.com) and the [Partner Center](https://partner.microsoft.com/dashboard/windows/overview).
